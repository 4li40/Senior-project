...
export default function ProfilePage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setProfile((prev) =>
        prev ? { ...prev, profilePictureUrl: `/api/profile/picture/${data.fileId}` } : prev
      );
    } catch (error) {
      console.error(error);
      alert("Upload failed");
  ...
  const handleCertificateUpload = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5003/api/profile/upload-certificate", {
        method: "POST",
        credentials: "include",
        body: formData,
      });
      ...
      setProfile((prev) =>
        prev && prev.certificateUrls
          ? {
              ...prev,
              certificateUrls: [...prev.certificateUrls, `/api/profile/certificate/${data.fileIds?.[0]}`],
            }
          : prev
      );
    } catch (error) {
  ...
  if (!profile) return <p className="p-6 text-center">Error loading profile.</p>;
  ...
  return (
    <div>
      ...
      <div className="max-w-4xl mx-auto p-6">
        <Button variant="outline" onClick={() => router.push(`/${profile.userType}-dashboard`)} className="mb-4">
          ...
          <CardContent className="flex flex-col md:flex-row items-center gap-6">
            <div className="w-32 h-32 rounded-full overflow-hidden border shadow">
              {preview || profile.profilePictureUrl ? (
                <img
                  src={preview || `http://localhost:5003${profile.profilePictureUrl}`}
                  alt="Profile"
        ...
        <div className="max-w-4xl mx-auto p-6 space-y-6">
          <Card>
            ...
            <CardContent className="space-y-2 text-sm text-gray-700">
              ...
              <p><strong>Subjects:</strong> {Array.isArray(profile.subjects) ? profile.subjects.join(", ") : "Not specified"}</p>
            </CardContent>
          </Card>

          <Card>
            <CardHeader>
              <CardTitle className="text-xl font-semibold">Edit Learning Goals</CardTitle>
            </CardHeader>
            <CardContent>
      ...
      {profile.userType === "tutor" && (
        <>
          <div className="max-w-4xl mx-auto p-6 space-y-6">
            <Card>
              <CardHeader>
                <CardTitle className="text-xl font-semibold">Subjects You Teach</CardTitle>
              </CardHeader>
              <CardContent>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex gap-4 items-center">
                  ...
                  <Button size="sm" onClick={handleCertificateUpload}>Upload</Button>
                </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-xl font-semibold">Update Profile Details</CardTitle>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleUpdate} className="space-y-4">
                  <div>
