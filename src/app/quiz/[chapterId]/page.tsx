...
export default function TakeQuizPage() {
  ...
  useEffect(() => {
    ...
    const fetchQuiz = async () => {
      try {
        const res = await fetch(`http://localhost:5003/api/quiz/${parsedChapterId}/questions`, {
          credentials: "include",
        });
    ...
    const checkAttempts = async () => {
      try {
        const res = await fetch(`http://localhost:5003/api/quiz/${parsedChapterId}/status`, {
          credentials: "include",
        });
  ...
  const handleSubmit = async () => {
    try {
      const res = await fetch(`http://localhost:5003/api/quiz/${chapterId}/submit`, {
        method: "POST",
        ...
        <CardContent className="space-y-6">
          {questions.length === 0 ? (
            ...
            questions.map((q, index) => {
              ...
              return (
                <div key={q.id} className="p-4 border rounded-md bg-gray-50 space-y-4">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            ...
            {!viewOnly && !submitted && (
              <Button onClick={handleSubmit} disabled={answers.some((a) => !a.selectedOption)}>
                Submit Quiz
              </Button>
            )}
            {submitted && retryAvailable && !viewOnly && (
              <Button variant="outline" onClick={() => {
                setAnswers(questions.map((q) => ({ questionId: q.id, selectedOption: "" })));
                setSubmitted(false);
                setScore(null);
