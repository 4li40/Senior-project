...
export default function AddQuestionForm({
  ...
  const validateBeforeSubmit = (): boolean => {
    for (const q of questions) {
      ...
      if (!isValidOption(q.options[parseInt(q.correct_option_index)]))
        return false;
    }
    return true;
  };
  ...
  const handleSubmit = async () => {
    if (!validateBeforeSubmit()) {
      alert(
        "🚫 All fields must be filled and options must be valid text/numbers."
      );
      return;
    }
    ...
    try {
      for (const q of questions) {
        const res = await fetch(
          "http://localhost:5003/api/quiz/tutor/add-question",
          {
            method: "POST",
          ...
          >
            ...
            <div>
              ...
              <RadioGroup
                ...
                onValueChange={(val) =>
                  handleQuestionChange(qIndex, "correct_option_index", val)
                }
              >
                {q.options.map((opt, optIndex) => (
